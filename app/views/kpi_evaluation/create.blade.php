                              ...
                              <?php foreach ($kpi->fields as $key => $value): ?>
								...
								<?php case "date": ?>
								 <div class="row">
										 ...
										 <div class="col-md-2">
											<div class="form-group {{{ $errors->has($value->field->name) ? 'has-error' : '' }}}">
											  <input type="text"  class="form-control datepicker"  name="{{$value->field->name}}" >
											{{ $errors->first($value->field->name, '<span class="help-block">:message</span>') }}
											</div>
										</div>
										</div>
					...
					@if(isset($kpi->fields))
                        ...
                        <table class="table table-hover table-bordered dt jambo_table bulk_action " >
							...
							<tbody>
								...
								?>
									<td>
								<a class="btn btn-sm btn-warning" href="{{URL::to('kpiEvaluation/edit/'.$result->id)}}"><i class="fa fa-pencil"></i></a>	
								<a class="btn btn-sm btn-danger" href="{{URL::to('kpiEvaluation/delete/'.$result->id)}}" onclick="return confirm('Are you sure?')"><i class="fa fa-remove"></i></a>
									</td>
							  <?php
									echo "</tr>";
								}
