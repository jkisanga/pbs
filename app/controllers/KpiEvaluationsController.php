...
class KpiEvaluationsController extends \BaseController {
	 ...
	 */
	 public function create($id)
 	{
		...
		// KPI Bar Graph data values per financial Year for every quarter for a given Unit
 		...
 		return View::make('kpi_evaluation.create',compact('kpi','results','submitted_quarters'));
 	}
	...
	public function edit($id)
 	{
 		$data = KpiEvaluation::find($id);
 		$kpi = Kpi::find($data->kpi_id);
		$financial_year = FinancialYear::where('status','=',true)->first();
		$user = Auth::user();
		$unit = Unit::find($user->unit_id);
		$submitted_quarters = $this->kpiControl->submitted_quarters($financial_year->id,$unit->id);
 		...
 		return View::make('kpi_evaluation.edit',compact('kpi','data','results','submitted_quarters'));
 	}
	
	public function update($id)
 	{
			$kpiEvaluation = KpiEvaluation::findOrFail($id);
			$kpi = Kpi::find(Input::get('kpi_id'));
			$data = Input::all();
		...
		if ($validator->passes())
		{
			 if($this->kpiControl->is_submitted(Input::get('year_id'),Auth::user()->unit_id,Input::get('quarter'))){
                return Redirect::back()->with('warning', 'Selected quarter evaluation already submitted');
					...
					$data['zone_id'] = $unit->zone_id;
					$data['user_id'] = $user->id;
					$kpiEvaluation->update($data);

			return Redirect::to('kpiEvaluation/create/'.$kpi->id)->with('success', 'Data has been updated');
			}
  ...
  // KPI zone Report
	...
	//Unit/ Station Report per Kpi
	public function getAllUnitKpiReport()
 	{
 		$objectives = Objective::orderBy('ob_code')->get();
		$years = FinancialYear::all();
		$f_year = FinancialYear::where('status','=',true)->first();		
		$unit = Unit::find(Auth::user()->unit_id);
		$zone = Zone::find($unit->zone_id);				
		$zones = Zone::where('deleted','=',0)->get();
								
		$view  = View::make('kpi_evaluation.unitKpiReport',['objectives'=>$objectives,'years'=>$years,'f_year'=>$f_year,'zone'=>$zone,'zones'=>$zones]);
	...
	public function postAllUnitKpiReport()
 	{
		$objectives = Objective::orderBy('ob_code')->get();
 		$KPIE = Kpi::find(Input::get('kpi_id'));
		$years = FinancialYear::all();
		$unit = Unit::find(Auth::user()->unit_id);
		$allunits = Unit::where('zone_id','=',$unit->zone_id)->where('deleted','=',0)->get();
		$f_year = FinancialYear::where('status','=',true)->first();		
		...
		if(isset($data['zone'])){
				$zone = Zone::find($data['zone']);
				$allunits = Unit::where('zone_id','=',$data['zone'])->where('deleted','=',0)->get();
			}
			
		$zones = Zone::where('deleted','=',0)->get();
											
		$view  = View::make('kpi_evaluation.unitKpiReport',['objectives'=>$objectives,'KPIE'=>$KPIE,'years'=>$years,'unit'=>$unit,'allunits'=>$allunits,'f_year'=>$f_year,'zone'=>$zone,'zones'=>$zones]);
